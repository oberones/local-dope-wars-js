...
- A typed content-pack foundation now includes a player-facing launch-screen picker, more than one bundled starter pack, and browser-local JSON import for custom packs.
...
- Import/export tooling around custom packs is still early and currently browser-local only.
...
3. Add more bundled packs and deepen the custom-pack tooling beyond the current browser-local import path.
